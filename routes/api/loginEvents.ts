...
router.get('/', function(req, res, next) {
    ...
    console.log('PARAMS: %s, %s, %s', userId, apiKey, deviceUserId);
    ...
    function executeStatement(res) {
        managers.dbManager.allowRequest(userId, apiKey, function(authorised) {
            if(!authorised) {
                ...
                var request = new Request("SELECT * FROM dbo.LoginEvents WHERE UserId = @UserId AND LoginTime > CAST(GETDATE() AS DATE) ORDER BY LoginTime DESC;", function(err, rowCount) {
                    if (err) {
                        console.log(err);
